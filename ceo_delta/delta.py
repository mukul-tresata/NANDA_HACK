...
The dominant term determines directive type:
    cascade     → replan  (structural failure)
    echo+miss   → replan  (combined confusion)
    echo        → replan  (differentiate intents)
    mismatch    → refine  (constrain roles)
    low_fp      → refine  (sharpen intents)
    high_error  → replan  (simplify)
    low_fp+drift→ replan  (missed replan window)
    Δe < thresh → surface (good enough)

When no rule fires (novel failure), Delta escalates to LLM fallback and
logs the escalation for future rule promotion.

Directive effectiveness is tracked in HandbookEntry.directive_outcomes:
    key = f"{action}:{reason_slug}"
    value = {"fires": int, "fixed": int, "delta_e_before": float, "delta_e_after": float}
"""
...
class DeltaReport:
    ...
    delta_e: float = 0.0           # scalar error signal
...
class Delta:
    ...
    def audit(
        ...
        satisfaction = {
            "signal": user_satisfaction if user_satisfaction is not None else -1.0
        }
        ...
        directive = self._generate_directive(
            ...
            dag.topology, dag.depth,
            delta_e, verdict, hub_failures,
            iteration,
        )
    ...
    def _generate_directive(
        ...
        2. If no rule matches and Δe is low -> surface.
        ...
        if hint:
            ...
            return DeltaDirective(
                action=action,
                reason=self._reason_slug(failure, semantic),
                replan_hint=hint,
                refinement_targets=targets,
                confidence=min(1.0, delta_e),
                iteration=iteration,
                escalated=False,
            )
    ...
    def _action_from_hint(self, failure: Dict, semantic: Dict, delta_e: float) -> str:
        """Cascade and high echo → replan. Role mismatch and low fp → refine."""
    ...
    def _reason_slug(self, failure: Dict, semantic: Dict) -> str:
        """Short slug identifying the dominant failure signal."""
        if failure["weighted_cascade_rate"] > 0.1:
            return "hub_cascade"
    ...
    def _refinement_targets(self, failure: Dict, structural: Dict) -> List[str]:
        """Return human-readable target descriptions, not node IDs (we don't have them here)."""
        targets = []
        if failure["weighted_echo_rate"] > 0.3:
    ...
    def _escalate(
        ...
        """No rule matched — log the escalation, optionally call LLM fallback."""
        escalation_entry = {
            "delta_e": round(delta_e, 4),
            "verdict": verdict,
            "failure": failure,
        ...
        return DeltaDirective(
            ...
            replan_hint=(
                f"Delta encountered an unclassified failure pattern "
                f"(Δe={delta_e:.3f}, verdict={verdict}). "
                f"Reconsider topology and node decomposition from first principles."
            ),
            confidence=0.5,
            iteration=iteration,
            escalated=True,
    ...
    def _log_escalation(self, entry: Dict) -> None:
        try:
            import os
    ...
    def _failure(
        self, dag: DAG, trace: ExecutionTrace, weights: Dict[str, float]
    ) -> Dict[str, float]:
        ...
        total_weight = (
            sum(weights.get(r.node_id, 1.0) for r in trace.results) or 1.0
        )
        weighted_err = sum(
            weights.get(r.node_id, 1.0) for r in trace.results if r.error
        ) / total_weight
        weighted_role_miss = sum(
            weights.get(r.node_id, 1.0)
            for r in trace.results
            if not r.role_function_match
        ) / total_weight
            ...
            for j in range(i + 1, len(nodes)):
                if set(nodes[i].dependencies) == set(nodes[j].dependencies):
                    ...
                    if a and b:
                        ...
                        total_w += pair_w
                        if (
                            cosine(a.output_embedding, b.output_embedding)
                            > self.cfg.echo_cosine_threshold
                        ):
                            echo_w += pair_w
        return echo_w / total_w if total_w > 0 else 0.0

    def _weighted_cascade_rate(
        self, dag: DAG, trace: ExecutionTrace, weights: Dict[str, float]
    ) -> float:
        by_id = {r.node_id: r for r in trace.results}
        ...
        for n in dag.nodes:
            ...
            if r and r.error:
                ...
                if errored_parents:
                    propagated_w += max(
                        weights.get(p, 1.0) for p in errored_parents
                    )
    ...
    def _echo_rate(self, dag: DAG, trace: ExecutionTrace) -> float:
        ...
        echoes = sum(
            1
            for a, b in sibs
            if cosine(a.output_embedding, b.output_embedding)
            > self.cfg.echo_cosine_threshold
        )
    ...
    def _granular(
        ...
        for r in trace.results:
            if r.cost_tokens > self.cfg.surprise_factor * planned:
                ...
                out.append(
                    f"per-node entry: {r.node_id} cost {r.cost_tokens} >> "
                    f"{planned:.0f} (centrality={w:.2f})"
                )
        ...
        if hub_failures:
            out.append(
                f"hub-failure entry: {hub_failures} — structurally critical nodes failed"
            )
        return out
    ...
    def _make_revision(self, dag, structural, failure, verdict, hub_failures) -> str:
        ...
        if hub_failures:
            tips.append(
                f"hub nodes {hub_failures} failed — add verification upstream of each hub"
            )
        if failure["weighted_echo_rate"] > 0.3:
